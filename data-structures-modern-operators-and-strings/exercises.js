...
const printGoals = function (...players) {
  ...
  console.log(`At least ${players.length} goals were scored.`);
}
// TEST DATA
...
let averageOdd = 0;
for (const odd of oddValues) averageOdd += odd;
...
// 4.
for (const [time, event] of gameEvents.entries()) console.log(`[${time <= 45 ? 'FIRST' : 'SECOND'} HALF] ${time}: ${event}`);
...
for (let [i, words] of inputArr.entries()) {
  words = words.toLowerCase().trim().split('_');
  for (let i = 1; i < words.length; i++) {
    words[i] = words[i].replace(words[i][0], words[i][0].toUpperCase());
  }
  const output = words.join('').padEnd(20) + '✅'.repeat(i + 1);
  console.log(output);
};